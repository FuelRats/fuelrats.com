...
const renderModal = ({ item, key, props: style }) => {
  const {
    as, className, hideClose, isOpen, onClose, title,
  } = item

  const { Component: InnerModal, children: innerModalChildren, props: innerModalProps } = item.children

  const RootElement = animated[as]

  const rootClasses = classNames('modal', [className, className])
...
const OuterModal = (props) => {
  const { hideClose, isOpen, onClose, onSafeUnmount } = props
  ...
  return <ModalPortal isOpen={isOpen}>{modalTransition.map(renderModal)}</ModalPortal>
}
...
const asModal = (options) => (Component) => hoistNonReactStatics(
  ({ children, ...props }) => (
    <OuterModal {...props} {...options}>
